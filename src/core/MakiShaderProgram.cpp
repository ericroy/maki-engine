...
namespace maki {
	namespace core {
		...
		bool shader_program_t::load(rid_t rid, shader_program_variant_t variant) {
			...
			input_attribute_count_ = input_attribute_count_node->value_as_uint(0);
			...
			if(!vertex_shader.init(vertex_shader_node, variant_data_key[variant], variant_meta_key[variant])) {
			...
			if(!fragment_shader.init(fragment_shader_node, variant_data_key[variant], variant_meta_key[variant])) {
				if(variant == variant_normal)
