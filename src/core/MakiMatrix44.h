...
namespace Maki
{
	...
	public:
		...
		inline static void Frustum(const Maki::Frustum &f, Matrix44 &m) { Matrix44::Frustum(f.left, f.right, f.bottom, f.top, f.nearPlane, f.farPlane, m); }
		
		static void Perspective(float fovY, float aspect, float nearPlane, float farPlane, Matrix44 &m);
		inline static void Perspective(const Maki::Frustum &f, Matrix44 &m) { Perspective(f.fov, f.GetAspect(), f.nearPlane, f.farPlane, m); }

		static void Ortho(float l, float r, float b, float t, float n, float f, Matrix44 &m);
		inline static void Ortho(const Maki::Frustum &f, Matrix44 &m) { Ortho(f.left, f.right, f.bottom, f.top, f.nearPlane, f.farPlane, m); }
