...
namespace maki {
	namespace core {
		...
		public:
			...
			struct controller_t {
			public:
				inline float get_float(button_t b) const {
					return values[mapping[b]];
				}
				
				inline bool get_bool(button_t b) const {
					return values[mapping[b]] == 1.0f;
				}
				
				inline void map_button(button_t hardware_button, button_t virtual_button) {
					mapping[virtual_button] = hardware_button;
				}
			...
			public:
				...
				float values[button_count];
				uint8_t mapping[button_count];
			};
			...
			public:
				...
				inline bool get_key(key_id_t k) const {
					return key_states_[key_mapping_[k]].on != 0;
				}
			
				// Returns true if the key became depressed during the last tick
				inline bool get_key_pressed(key_id_t k) const {
					return key_states_[key_mapping_[k]].pressed != 0;
				}

				// Returns true if the key was released during the last tick
				inline bool get_key_released(key_id_t k) const {
					return key_states_[key_mapping_[k]].released != 0;
				}

				inline void map_key(key_id_t hardware_key, key_id_t virtual_key) {
				...
				void set_key_states_(const key_state_report_t states[256]) {
					for(uint32_t i = 0; i < 256; i++) {
