...
namespace maki {
	namespace core {
		...
		public:
			...
			inline void merge(const vector4_t &point) {
				merge(point.x, point.y, point.z);
			}

			inline void merge(const vector3_t &point) {
				merge(point.x, point.y, point.z);
			}
			...
			inline vector4_t get_max_corner() const {
				return pos + radii;
			}

			inline vector4_t get_min_corner() const {
				return pos - radii;
			}
			...
			inline float get_radius_sq() const {
				return radii.length_squared();
			}
