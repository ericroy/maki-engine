...
namespace Maki
{
	...
	Direct3DRenderCore::Direct3DRenderCore(Window *window, const Config *config)
		:	RenderCore(),
			swapChain(nullptr),
			device(nullptr),
			context(nullptr),
			...
			blendDisabled(nullptr)
	{
		...
		scd.OutputWindow = ((Win32Window *)window)->hwnd;
		scd.SampleDesc.Count = 1;
		scd.SampleDesc.Quality = 0;
		scd.Windowed = !((Win32Window *)window)->fullscreen;
	...
	Direct3DRenderCore::~Direct3DRenderCore() {
	...
	void Direct3DRenderCore::Resized(uint32 newWidth, uint32 newHeight)
	{
	...
	void Direct3DRenderCore::Present()
	{
		std::lock_guard<std::mutex> lock(mutex);
		swapChain->Present(vsync ? 1 : 0, 0);
	}

	void Direct3DRenderCore::Draw(const RenderState &state, const DrawCommandList &commands)
	{
		std::lock_guard<std::mutex> lock(mutex);
	...
	void Direct3DRenderCore::SetDepthState(RenderState::DepthTest test, bool write)
	{
		switch(test) {
	...
	void Direct3DRenderCore::SetRasterizerState(RenderState::CullMode cullMode, bool wireFrame)
	{
		switch(cullMode) {
	...
	void Direct3DRenderCore::SetRenderTargetAndDepthStencil(RenderState::RenderTarget renderTargetType, Handle renderTarget, RenderState::DepthStencil depthStencilType, Handle depthStencil)
	{
		currentRenderTargetView = nullptr;
	...
	void Direct3DRenderCore::SetPerFrameConstants(const RenderState &state, const Shader *s, D3D11_MAPPED_SUBRESOURCE &mapped)
	{
	...
	void Direct3DRenderCore::SetPerObjectConstants(const Shader *s, D3D11_MAPPED_SUBRESOURCE &mapped, const Matrix44 &model, const Matrix44 &modelView, const Matrix44 &modelViewProjection)
	{
	...
	void Direct3DRenderCore::BindMaterialConstants(const Shader *s, bool isVertexShader, D3D11_MAPPED_SUBRESOURCE &mapped, const Material *mat)
	{
	...
	void Direct3DRenderCore::BindTextures(const ShaderProgram *shader, const TextureSet *ts)
	{
	...
	void *Direct3DRenderCore::UploadBuffer(void *buffer, VertexFormat *vf, char *vertexData, uint32 vertexCount, char *indexData, uint32 faceCount, uint8 indicesPerFace, uint8 bytesPerIndex, bool dynamic)
	{
		std::lock_guard<std::mutex> lock(mutex);
	...
	void Direct3DRenderCore::FreeBuffer(void *buffer)
	{
		std::lock_guard<std::mutex> lock(mutex);
	...
	bool Direct3DRenderCore::CreatePixelShader(Shader *ps)
	{
...
failed:
	...
	bool Direct3DRenderCore::CreateVertexShader(Shader *vs)
	{
...
failed:
	...
	bool Direct3DRenderCore::CreateShaderProgram(ShaderProgram *s)
	{
		std::lock_guard<std::mutex> lock(mutex);
	...
	bool Direct3DRenderCore::CreateEmptyTexture(Texture *t, uint8 channels)
	{
		std::lock_guard<std::mutex> lock(mutex);
...
failed:		
	...
	bool Direct3DRenderCore::CreateRenderTarget(Texture *t)
	{
		std::lock_guard<std::mutex> lock(mutex);
...
failed:		
	...
	bool Direct3DRenderCore::CreateDepthTexture(Texture *t)
	{
		std::lock_guard<std::mutex> lock(mutex);
...
failed:		
	...
	bool Direct3DRenderCore::CreateTexture(Texture *t, char *data, uint32 dataLength)
	{
		std::lock_guard<std::mutex> lock(mutex);
...
failed:		
	...
	void Direct3DRenderCore::WriteToTexture(Texture *t, int32 dstX, int32 dstY, int32 srcX, int32 srcY, uint32 srcWidth, uint32 srcHeight, uint32 srcPitch, uint8 channels, char *srcData)
	{
		std::lock_guard<std::mutex> lock(mutex);
	...
	void Direct3DRenderCore::DeleteShaderProgram(ShaderProgram *s)
	{
		std::lock_guard<std::mutex> lock(mutex);
	...
	void Direct3DRenderCore::DeleteTexture(Texture *t)
	{
		std::lock_guard<std::mutex> lock(mutex);
...
#if 0
	void Direct3DRenderCore::GetAdapterInfo(uint32 windowWidth, uint32 windowHeight) {
