		...
		namespace Win32
		{
			...
			LRESULT CALLBACK WndProcDispatcher(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
			{
				...
				if(message == WM_CREATE) {
					instance = (WindowImpl *)((CREATESTRUCT *)lparam)->lpCreateParams;
					SetWindowLongPtr(hwnd, 0, reinterpret_cast<LONG_PTR>(instance));
				} else {
					instance = reinterpret_cast<WindowImpl *>(GetWindowLongPtr(hwnd, 0));
				}
			...
			WindowImpl::WindowImpl(HINSTANCE hinst, LPCTSTR title, const Config *config)
			...
			int WindowImpl::Pump(Engine *engine) {
				MSG msg;
			...
			LRESULT WindowImpl::WndProc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) {
				switch (message) {
				case WM_CREATE:
					this->hwnd = hwnd;
					hdc = GetDC(hwnd);
			...
			void WindowImpl::PollInput(InputState *state) {
