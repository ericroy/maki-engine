...
namespace maki {
	namespace core {
		...
		void renderer_t::sync_with_core() {
			auto outstanding = max_render_payloads - render_states_.size() - 1;
			while(outstanding != 0) {
		...
		void renderer_t::set_render_target(render_state_t::render_target_t render_target_type, const ref_t<texture_t> &render_target) {
			current_.render_target_type = render_target_type;
			if(render_target_type == render_state_t::render_target_custom)
				current_.render_target = render_target;
			else
				current_.render_target.release();
		}

		void renderer_t::set_depth_stencil(render_state_t::depth_stencil_t depth_stencil_type, const ref_t<texture_t> &depth_stencil) {
			current_.depth_stencil_type = depth_stencil_type;
			if(depth_stencil_type == render_state_t::depth_stencil_custom)
				current_.depth_stencil = depth_stencil;
			else
				current_.depth_stencil.release();
		}

		void renderer_t::set_ortho_projection(const frustum_t &frustum) {
			current_.camera_width_height_near_far = vector4_t(frustum.width(), frustum.height(), frustum.near(), frustum.far());
			matrix44_t::ortho(frustum, current_.projection);
		}

		void renderer_t::set_perspective_projection(const frustum_t &frustum) {
			current_.camera_width_height_near_far = vector4_t(frustum.width(), frustum.height(), frustum.near(), frustum.far());
			matrix44_t::perspective(frustum, current_.projection);
		}

		void renderer_t::set_light(uint32_t light_index, const render_state_t::light_properties_t *props, const render_state_t::shadow_map_properties_t *shad_props, const matrix44_t *matrix, float fov, const ref_t<texture_t> &depth_buffer) {
			light_dirty_flags_ |= (1<<light_index);

			if(props == nullptr || (props->flags & render_state_t::light_flag_on) == 0) {
				current_.light_properties[light_index].flags &= ~render_state_t::light_flag_on;
				memset(&current_.light_properties[light_index], 0, sizeof(render_state_t::light_properties_t));
				current_.shadow_maps[light_index].release();
				return;
			}

			memcpy(&current_.light_properties[light_index], props, sizeof(render_state_t::light_properties_t));
			...
			if(fov == 0.0f) {
				float wo2 = props->width_height_near_far.x / 2.0f;
				float ho2 = props->width_height_near_far.y / 2.0f;
				matrix44_t::ortho(-wo2, wo2, -ho2, ho2, props->width_height_near_far.z, props->width_height_near_far.w, current_.light_proj[light_index]);
			} else {
				matrix44_t::perspective(fov, props->width_height_near_far.x / props->width_height_near_far.y, props->width_height_near_far.z, props->width_height_near_far.w, current_.light_proj[light_index]);
			}
		
			if((props->flags & render_state_t::light_flag_shadow) != 0 && depth_buffer) {
				MAKI_ASSERT(light_index < render_state_t::max_shadow_lights);
				current_.shadow_maps[light_index] = depth_buffer;
				current_.shadow_map_properties[light_index] = *shad_props;
			} else {
				if(light_index < render_state_t::max_shadow_lights) {
					current_.shadow_map_properties[light_index].size = vector2_t(0.0f);
					current_.shadow_maps[light_index].release();
				}
			}
		}
		...
		void renderer_t::set_light_cascade(uint32_t light_index, uint32_t cascade_index, const frustum_t &frustum) {
			MAKI_ASSERT(light_index < current_.cascaded_shadow_light_count);
			MAKI_ASSERT(cascade_index < render_state_t::max_cascades);

			render_state_t::light_split_region_t &region = current_.light_split_regions[light_index][cascade_index];

			matrix44_t::ortho(frustum, region.view_proj);

			// Light must have been set already!!!!!!
			region.view_proj = region.view_proj * current_.light_view[light_index];
		
			region.width_height_near_far = vector4_t(frustum.width(), frustum.height(), frustum.near(), frustum.far());
		}
		...
		void renderer_t::submit() {
			...
			//if(light_dirty_flags_ != 0) {
				...
				for(uint32_t i = 0; i < current_.light_count; i++) {
					//if((light_dirty_flags_ & (1<<i)) != 0 && (current_.light_properties_[i].flags_ & render_state_t::light_flag_on_) != 0) {
						matrix44_t to_view_space = current_.view * current_.light_world[i];
						current_.light_positions[i] = to_view_space * pos;
						current_.light_directions[i] = to_view_space * dir;
