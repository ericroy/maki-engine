...
template<typename FT> inline FT GetOGLFunc(FT f, const char *name) {
	...
	if(val == nullptr) {
		Maki::core::console_t::warning("Failed to get address of OpenGL function: %s", name);
	}
	return val;
}
...
namespace maki
{
	namespace ogl
	{
		...
		bool __failed()
		{
			...
			if(error != GL_NONE) {
				console_t::error("OGL: %s", (const char *)gluErrorString(error));
				return true;
			}
			return false;
		}

		const GLenum indices_per_face_to_geometry_type[4] = {
			GL_INVALID_ENUM,
			GL_POINTS,
			GL_LINES,
			GL_TRIANGLES,
		...
		const GLenum bytes_per_index_to_format[5] = {
			GL_INVALID_ENUM,
			GL_INVALID_ENUM,
			GL_UNSIGNED_SHORT,
			GL_INVALID_ENUM,
		...
		const GLenum channels_to_format[5] = {
			GL_INVALID_ENUM,
			GL_ALPHA,
			GL_RG,
		...
		const GLenum type_to_gl_type[core::vertex_format_t::data_type_count_] = {
		...
		const char *attribute_name[core::vertex_format_t::attribute_count_] = {
			"Position",
			"Normal",
			"Tangent",
			"Color",
		...
		GLboolean normalize_attribute[core::vertex_format_t::attribute_count_] = {
			GL_FALSE,
			GL_FALSE,
			GL_FALSE,
			GL_TRUE,
